...
export default function QuotePage() {
  ...
  useEffect(() => {
    settingsApi.get('company_config')
      .then(r => {
        if (r.data?.value) {
          const cfg = r.data.value;
          setBiz({
            name:       cfg.name       || '',
            address:    [cfg.address, cfg.city, cfg.province, cfg.postal].filter(Boolean).join(', '),
            email:      cfg.email      || '',
            phone:      cfg.phone      || '',
            hst_number: cfg.hst_number || '',
          });
          setHstEnabled(cfg.enable_hst !== false);
          setHstRate(parseFloat(cfg.hst_rate) || 13);
        } else {
          setBiz({ name: '', address: '', email: '', phone: '', hst_number: '' });
        }
      })
      .catch(() => setBiz({ name: '', address: '', email: '', phone: '', hst_number: '' }));
  ...
  function fillFromOrder(id) {
    ...
    setLines([{ description: o.description || 'Custom 3D Print', qty: 1, unit_price: o.price_cad || '', hst: hstEnabled }]);
  }
          ...
          <div style={{ marginBottom: 16, padding: '12px 16px', background: 'var(--bg-hover)', borderRadius: 8, border: '0.5px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)' }}>
                {biz.name || <span style={{ color: 'var(--text-tertiary)' }}>No business name set</span>}
              </div>
              <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 2 }}>
                {[biz.address, biz.email, biz.phone].filter(Boolean).join(' · ')}
                {biz.hst_number && ` · HST #${biz.hst_number}`}
                {hstEnabled && ` · Tax ${hstRate}%`}
              </div>
            </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div className="card" style={{ padding: 18 }}>
            <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
              {['quote', 'invoice'].map(t => (
                <button key={t} className={`btn btn-sm ${docType === t ? 'btn-primary' : 'btn-secondary'}`}
                  onClick={() => setDocType(t)} style={{ textTransform: 'capitalize', minWidth: 100, justifyContent: 'center' }}>
          ...
          <div className="card" style={{ padding: 18 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <select className="select" style={{ fontSize: 12 }} onChange={e => fillFromCustomer(e.target.value)} value="">
                  <option value="">Fill from customer…</option>
                  {customers.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                </select>
                <select className="select" style={{ fontSize: 12 }} onChange={e => fillFromOrder(e.target.value)} value={linkedOrder || ''}>
                  <option value="">Fill from order…</option>
                  {orders.filter(o => o.status !== 'cancelled').slice(0, 100).map(o => (
          ...
          <div className="card" style={{ padding: 18 }}>
            ...
            {lines.map((l, i) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 60px 110px 60px 28px', gap: 6, marginBottom: 6 }}>
                ...
                <button onClick={() => removeLine(i)} style={{ background: 'none', border: 'none', color: 'var(--red)', cursor: 'pointer', fontSize: 16, lineHeight: 1, padding: 0 }}>✕</button>
              </div>
            ))}
            ...
            <div style={{ marginTop: 16, paddingTop: 14, borderTop: '0.5px solid var(--border)', maxWidth: 280, marginLeft: 'auto' }}>
              ...
              {hstEnabled && (
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, padding: '4px 0', color: 'var(--text-secondary)' }}>
                  <span>Tax ({hstRate}%)</span><span>{formatCAD(hstAmount)}</span>
                </div>
              )}
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 16, fontWeight: 700, padding: '10px 0 0', color: 'var(--accent)', borderTop: '0.5px solid var(--border)', marginTop: 6 }}>
...
// ── HTML template ────────────────────────────────────────────────────────────
function buildHTML({ docType, docNum, docDate, dueDate, customer, lines, notes, biz, subtotal, hstAmount, total, hstRate, hstEnabled }) {
  ...
  const lineRows = lines.filter(l => l.description).map(l => {
    ...
    return `
      <tr>
        ...
        ${hstEnabled ? `<td style="text-align:center">${l.hst ? '✓' : '—'}</td>` : ''}
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .bill-section { display: flex; gap: 40px; margin-bottom: 28px; padding: 18px 20px; background: #f8f9ff; border-radius: 10px; }
    ...
    .totals-row.grand { font-size: 16px; font-weight: 800; color: #0071e3; border-top: 2px solid #0071e3; padding-top: 10px; margin-top: 4px; }
    .notes { margin-top: 28px; padding: 14px 18px; background: #f8f9ff; border-radius: 8px; border-left: 4px solid #0071e3; }
    ...
    .footer { margin-top: 36px; padding-top: 14px; border-top: 1px solid #eee; font-size: 11px; color: #999; text-align: center; }
...
<body>
  ...
  <div class="bill-section">
    ...
    <div>
      ...
      <div class="bill-detail">E-Transfer to:<br><strong>${biz.email}</strong></div>
    </div>` : ''}
  </div>
  ...
  <table>
    <thead>
      <tr>
        ...
        ${hstEnabled ? `<th style="text-align:center;width:60px">Tax</th>` : ''}
  ...
  <div class="totals">
    <div class="totals-box">
      ...
      ${hstEnabled ? `<div class="totals-row"><span>Tax (${hstRate}%)</span><span>$${hstAmount.toFixed(2)}</span></div>` : ''}
  ...
  <div class="footer">${biz.name || ''} · ${biz.email || ''} · Generated by PrintFlow</div>
</body>
</html>`;
}
